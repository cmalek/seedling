...
RUN pip install -e . && \
    ...
    DATABASE_URL='' ./manage.py collectstatic --settings=config.settings.local --noinput -v0 -i node_modules --link && \
    ...
    cp etc/apache/seedling.conf /etc/httpd/conf.d/seedling.conf && \
