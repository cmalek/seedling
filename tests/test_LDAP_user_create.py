...
from djunk.utils import FakeCurrentRequest
from testfixtures import Replacer, LogCapture
from wagtail.core.models import Site

from caltech_sites.models import ContactInformation
from core.tests.utils import MultitenantSiteTestingMixin, SecureClientMixin
from wagtail_patches.views.users import create
from ..forms import LDAPUserCreateForm
...
class TestLDAPUserCreate(SecureClientMixin, TestCase, MultitenantSiteTestingMixin):
    ...
    def setUpTestData(cls):
        ...
        cls.wagtail_factory = RequestFactory(**{
            ...
            'SERVER_NAME': 'wagtail.flint.caltech.edu',
        })
        cls.test_factory = RequestFactory(**{
            'wsgi.url_scheme': 'https',
            'SERVER_NAME': 'test.flint.caltech.edu',
        })
    ...
    def test_create_brand_new_LDAP_nonsuperuser_happy_path(self):
        ...
        request.site = Site.objects.get(hostname='wagtail.flint.caltech.edu')
        # Should create a new user named "wagtail_user1" in the "wagtail.flint.caltech.edu Admins" Group.
        username = 'wagtail_user1'
        contact_info = ContactInformation(uid=username, first_name='John', last_name='Snow', email='uknow@nothing.com')
        contact_info.save()
        form_data = {'contact_info': contact_info.pk, 'groups': [self.wagtail_admins_group.pk]}
        ...
        self.assertEqual(new_user.email, 'uknow@nothing.com')
        self.assertEqual(new_user.first_name, 'John')
        self.assertEqual(new_user.last_name, 'Snow')
        self.assertTrue(new_user.is_member_of_site(request.site))
    ...
    def test_create_brand_new_LDAP_superuser_happy_path(self):
        ...
        username = 'superuser1'
        contact_info = ContactInformation(uid=username, first_name='Super', last_name='User', email='uknow@nothing.com')
        contact_info.save()
        form_data = {'contact_info': contact_info.pk, 'is_superuser': True}
        ...
        self.assertEqual(new_user.email, contact_info.email)
        self.assertEqual(new_user.first_name, contact_info.first_name)
        self.assertEqual(new_user.last_name, contact_info.last_name)
        self.assertFalse(new_user.is_member_of_site(request.site))
    ...
    def test_superuser_has_to_assign_LDAP_user_to_group_or_set_superuser(self):
        ...
        contact_info = ContactInformation(uid=username, first_name='John', last_name='Snow', email='uknow@nothing.com')
        contact_info.save()
        form_data = {'contact_info': contact_info.pk}
    ...
    def test_LDAP_nonsuperuser_cannot_create_new_LDAP_superuser(self):
        ...
        contact_info = ContactInformation(uid=username, first_name='John', last_name='Snow', email='uknow@nothing.com')
        contact_info.save()
        form_data = {'contact_info': contact_info.pk, 'is_superuser': True, 'groups': [self.wagtail_admins_group.pk]}
    ...
    def test_LDAP_duplicate_username_error(self):
        ...
        request.user = get_user_model().objects.get(username='wagtail_admin')
        request.site = Site.objects.get(hostname='wagtail.flint.caltech.edu')
        username = 'wagtail_editor'
        contact_info = ContactInformation(uid=username, first_name='John', last_name='Snow', email='uknow@nothing.com')
        contact_info.save()
        form_data = {'contact_info': contact_info.pk, 'groups': [self.wagtail_editors_group.pk]}
    ...
    def test_create_LDAP_user_belonging_to_other_site_groups_is_disallowed(self):
        ...
        contact_info = ContactInformation(uid=username, first_name='John', last_name='Snow', email='uknow@nothing.com')
        contact_info.save()
        form_data = {'contact_info': contact_info.pk, 'groups': [self.test_editors_group.pk]}
    ...
    def test_create_LDAP_user_which_already_exists(self):
        ...
        request.user = get_user_model().objects.get(username='wagtail_admin')
        request.site = Site.objects.get(hostname='wagtail.flint.caltech.edu')
        test_site = Site.objects.get(hostname='test.flint.caltech.edu')
        username = 'test_admin'
        contact_info = ContactInformation(uid=username, first_name='John', last_name='Snow', email='uknow@nothing.com')
        contact_info.save()
        ...
        self.assertTrue(existing_user.is_member_of_site(test_site))
        self.assertFalse(existing_user.is_member_of_site(request.site))
        ...
        self.assertTrue(new_user.is_member_of_site(test_site))
        self.assertTrue(new_user.is_member_of_site(request.site))
    ...
    def test_create_LDAP_user_view_GET(self):
        ...
        with Replacer() as r:
            ...
            r.replace('wagtail.admin.messages', self.messages_dummy)
            ...
            request.user = get_user_model().objects.get(username='wagtail_admin')
            request.site = Site.objects.get(hostname='wagtail.flint.caltech.edu')
            with FakeCurrentRequest(request=request):
