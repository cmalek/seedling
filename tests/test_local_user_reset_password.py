...
from wagtail_patches.views.users import admin_reset_password
from ..forms import LocalUserAdminResetPasswordForm
...
class TestLocalUserResetPassword(SecureClientMixin, TestCase, MultitenantSiteTestingMixin):
    ...
    def test_form_save(self):
        ...
        mail_dummy = Dummy()
        with Replacer() as r:
            r.replace('custom_auth.forms.LocalUserAdminResetPasswordForm.send_mail', mail_dummy)
    ...
    def test_local_user_reset_password_view_GET(self):
        ...
        with Replacer() as r:
            ...
            response = self.client.get(
                reverse('wagtailusers_users:admin_reset_password', args=['wagtail_editor_local']),
            ...
            with FakeCurrentRequest(request=request):
                response = admin_reset_password(request, 'wagtail_editor_local')
            ...
            self.assertEqual(response.context_data['user'].pk, self.wagtail_editor_local.pk)
    ...
    def test_local_user_reset_password_view_POST_invalid_data(self):
        ...
        with Replacer() as r:
            ...
            response = self.client.post(
                reverse('wagtailusers_users:admin_reset_password', args=['wagtail_editor_local']), data=form_data,
                HTTP_HOST=self.wagtail_site.hostname
            )
            self.assertEqual(response.status_code, 200)
            self.assertTemplateUsed(response, 'wagtail_patches/users/admin_reset_password.html')
            ...
            request = self.wagtail_factory.post(
                reverse('wagtailusers_users:admin_reset_password', args=[self.wagtail_editor_local.username]), data=form_data,
                HTTP_HOST=self.wagtail_site.hostname
            )
            request.user = self.wagtail_admin
            request.site = self.wagtail_site
    ...
    def test_edit_local_user_view_POST_valid_data(self):
        ...
        with Replacer() as r:
            ...
            form_data = {'username': self.wagtail_editor_local.username, 'email': 'blah@blah.com'}
            request = self.wagtail_factory.post(
                reverse('wagtailusers_users:admin_reset_password', args=[
                    self.wagtail_editor_local.username
                ]), data=form_data
            )
            request.user = self.wagtail_admin
            request.site = self.wagtail_site
